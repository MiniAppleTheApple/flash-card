...
const Decks: React.FC<{removeDeck: (deckIndex: number) => void, deckOnEdit: (index: number) => void, deckOnClick: (index: number) => void, decks: Deck[]}> = ({removeDeck, deckOnEdit, deckOnClick, decks}) => (
  <ul>
    {decks.map(
      (x, index) => 
        <li key={index} onClick={_e => deckOnClick(index)} className="rounded-lg bg-lunar-green-700 p-4 flex hover:bg-lunar-green-900 transition-colors my-2 cursor-pointer hover:text-lunar-green-700">
